...
CASE [hazardrate]
    ...
    WHEN 'GERIATRICPROCEDURES' THEN
        1000
            * ATTR(1 + DATEDIFF('day', [CENSUSSTART], [CENSUSEND]))
            * SUM([GERIATRICPROCEDURES])
            / SUM([DURATIONDAYS])
    WHEN 'GERIATRICIANSDAYS' THEN
        1000
            * ATTR(1 + DATEDIFF('day', [CENSUSSTART], [CENSUSEND]))
            * SUM([GERIATRICIANSDAYS])
            / SUM([DURATIONDAYS])
    ...
    WHEN 'GERIATRICPERCENTUTILIZATION' THEN
        100
            * SUM(IIF([GERIATRICPROCEDURES] > 0, [DURATIONDAYS], 0))
            / SUM([DURATIONDAYS])
    ...
    WHEN 'PEDIATRICPROCEDURES' THEN
        1000
            * ATTR(1 + DATEDIFF('day', [CENSUSSTART], [CENSUSEND]))
            * SUM([PEDIATRICPROCEDURES])
            / SUM([DURATIONDAYS])
    WHEN 'PEDIATRICIANSDAYS' THEN
        1000
            * ATTR(1 + DATEDIFF('day', [CENSUSSTART], [CENSUSEND]))
            * SUM([PEDIATRICIANSDAYS])
            / SUM([DURATIONDAYS])
    ...
    WHEN 'PEDIATRICPERCENTUTILIZATION' THEN
        100
            * SUM(IIF([PEDIATRICPROCEDURES] > 0, [DURATIONDAYS], 0))
            / SUM([DURATIONDAYS])

    WHEN 'PEDIATRICSURGICALPROCEDURES' THEN
        1000
            * ATTR(1 + DATEDIFF('day', [CENSUSSTART], [CENSUSEND]))
            * SUM([PEDIATRICSURGICALPROCEDURES])
            / SUM([DURATIONDAYS])
    WHEN 'PEDIATRICSURGEONSDAYS' THEN
        1000
            * ATTR(1 + DATEDIFF('day', [CENSUSSTART], [CENSUSEND]))
            * SUM([PEDIATRICSURGEONSDAYS])
            / SUM([DURATIONDAYS])
    ...
    WHEN 'PEDIATRICSURGERYDAYS' THEN
        1000
            * ATTR(1 + DATEDIFF('day', [CENSUSSTART], [CENSUSEND]))
            * SUM([PEDIATRICSURGERYDAYS])
            / SUM([DURATIONDAYS])
    WHEN 'PEDIATRICSURGERYPERCENTUTILIZATION' THEN
        100
            * SUM(IIF([PEDIATRICSURGICALPROCEDURES] > 0, [DURATIONDAYS], 0))
            / SUM([DURATIONDAYS])

    WHEN 'PSYCHIATRYPROCEDURES' THEN
        1000
            * ATTR(1 + DATEDIFF('day', [CENSUSSTART], [CENSUSEND]))
            * SUM([PSYCHIATRYPROCEDURES])
            / SUM([DURATIONDAYS])
    WHEN 'PSYCHIATRISTSDAYS' THEN
        1000
            * ATTR(1 + DATEDIFF('day', [CENSUSSTART], [CENSUSEND]))
            * SUM([PSYCHIATRISTSDAYS])
            / SUM([DURATIONDAYS])
    ...
    WHEN 'PSYCHIATRYPERCENTUTILIZATION' THEN
        100
            * SUM(IIF([PSYCHIATRYPROCEDURES] > 0, [DURATIONDAYS], 0))
            / SUM([DURATIONDAYS])
