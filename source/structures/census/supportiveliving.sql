CREATE MATERIALIZED VIEW censussupportiveliving NOLOGGING NOCOMPRESS NOCACHE PARALLEL 8 BUILD DEFERRED REFRESH COMPLETE ON DEMAND AS
SELECT
...
FROM
	...
	(
		...
		(
			a1.entry_from_date,
			COALESCE(a1.exit_to_date, a0.extremumend),
			a0.birthdate
		)
	) a2
GROUP BY
